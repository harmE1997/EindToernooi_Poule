...
namespace EindToernooi_Poule.ViewModels
{
    public class TotoFormVm : ViewModelBase
    {
        ...
        public TotoFormVm(Player activeplayer, Window totoformwindow)
        {
            ...
            var NextPouleCommandCanExecute = this.WhenAnyValue(
                x => x.CurrentPoule,
                (a) => { return a < LocalConfiguration.NrPoules; }).ObserveOn(RxApp.MainThreadScheduler);

            var PreviousPouleCommandCanExecute = this.WhenAnyValue(
                x => x.CurrentPoule,
            ...
            NextPouleCommand = ReactiveCommand.Create(() => { this.ChangePoule(1); }, NextPouleCommandCanExecute);
            PreviousPouleCommand = ReactiveCommand.Create(() => { this.ChangePoule(-1); }, PreviousPouleCommandCanExecute);
            ...
            Last32Active = LocalConfiguration.Last32;
            Last32InPlay = LocalConfiguration.Last32;
            BronzeInPlay = LocalConfiguration.Bronze;
            NlPresent = LocalConfiguration.NlPresent;
        ...
        private void ChangePoule(int change)
        {
            CurrentPoule += change;
        }
        ...
        private Player CreateDefaultActivePlayer()
        {
            ...
            for (int i = 1; i <= LocalConfiguration.NrPoules; i++)
            {
                ...
                poules.Add(i, new Poule(i, matches));
            }
            return new Player("", "", poules, new KnockoutPhase(), new BonusQuestions(new Dictionary<string, int>()));
        }
